...
import { menuListClassName, menuRootClassName } from '@fex-design/styles/menu'
...
const emit = defineEmits<{
  expandChange: [keys: MenuKey[], info: MenuRenderItemInfo]
  select: [keys: MenuKey[], info: MenuRenderItemInfo]
}>()
const menu = useMenu({
  items: () => props.items,
...
provide(menuUiContextKey, {
  classNames: props.class,
  entryMap: () => entryMap.value,
  indent: props.indent,
  menu,
...
<template>
  ...
  >
    <div
      ...
      data-slot="menu-list"
      :data-orientation="props.orientation"
      :class="cn(menuListClassName({ orientation: props.orientation }), props.class?.list)"
    >
      <MenuItems :items="props.items">
        <template v-if="$slots.item" #item="{ info }"><slot name="item" :info="info" /></template>
        <template v-if="$slots.suffix" #suffix="{ info }"
          ><slot name="suffix" :info="info"
        /></template>
        <template v-if="$slots['expand-icon']" #expand-icon="{ info }"
          ><slot name="expand-icon" :info="info"
        /></template>
      </MenuItems>
    </div>
  </div>
