...
export function Menu(props: MenuProps) {
  const menu = createMenu({
    ...
    ...(props.defaultExpandKeys === undefined
      ? {}
    ...
    return (
      <div data-slot="menu-node">
        <button
          ...
          class={cn(
            menuItemClassName({ orientation: props.orientation ?? 'vertical' }),
            props.class?.item,
          )}
          style={
            props.orientation === 'horizontal'
              ? undefined
              : {
                  'padding-left': `calc(var(--menu-item-padding-x) + ${info().level * (props.indent ?? 18)}px)`,
                }
          }
        ...
        >
          {props.renderItem?.(info()) ?? (
            <>
              {info().item.icon ? (
                <span class={cn(menuItemIconClassName, props.class?.icon)}>{info().item.icon}</span>
              ) : null}
              <span class={cn(menuItemLabelClassName, props.class?.label)}>
                {info().item.label}
              </span>
              {suffix() ? (
                <span class={cn(menuItemSuffixClassName, props.class?.suffix)}>{suffix()}</span>
              ) : null}
              {info().hasChildren ? (
                <span class={cn(menuExpandIconClassName, props.class?.expandIcon)}>
                  {props.renderExpandIcon?.(info()) ?? <ChevronRightIcon />}
                </span>
              ) : null}
            </>
          )}
        ...
        {info().hasChildren ? (
          <div
            class={cn(menuSubMenuContentClassName, props.class?.submenuContent)}
            data-expanded={info().expanded ? 'true' : 'false'}
            ...
            if (!isMenuGroupItem(item))
              return (
                <div
                  ...
                  class={cn(menuDividerClassName, props.class?.divider)}
                />
              )
            return (
              <div
                ...
                class={cn(menuGroupClassName, props.class?.group)}
              >
                {item.label ? (
                  <div
                    data-slot="menu-group-label"
                    class={cn(menuGroupLabelClassName, props.class?.groupLabel)}
                  >
                    {item.label}
                  </div>
                ) : null}
  ...
  return (
    <div
      ...
      class={cn(menuRootClassName({}), props.class?.root)}
    >
      <div
        role="group"
        data-slot="menu-list"
        data-orientation={props.orientation ?? 'vertical'}
        class={cn(
          menuListClassName({ orientation: props.orientation ?? 'vertical' }),
          props.class?.list,
        )}
      >
