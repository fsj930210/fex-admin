...
import type { OverlayTrigger } from '@fex-design/core/overlay/trigger/create-trigger'
...
@Component({
  ...
  host: {
    ...
    '[class]': 'hostClassName()',
    '[style.left]':
      "popover.snapshot().side === 'left' || popover.snapshot().side === 'right' ? null : 'clamp(var(--popover-arrow-inset,32px), var(--floating-arrow-x,50%), calc(100% - var(--popover-arrow-inset,32px)))'",
    '[style.top]':
      "popover.snapshot().side === 'left' || popover.snapshot().side === 'right' ? 'clamp(var(--popover-arrow-inset,32px), var(--floating-arrow-y,50%), calc(100% - var(--popover-arrow-inset,32px)))' : null",
    '[attr.data-side]': 'popover.snapshot().side',
  },
  template: '',
})
