...
<template><div v-bind="$attrs" data-slot="input-root" :data-disabled="input.disabled.value || undefined" :data-readonly="input.readOnly.value || undefined" :data-invalid="input.invalid.value || undefined" :class="className" @click="emit('click', $event)"><slot /></div></template>
