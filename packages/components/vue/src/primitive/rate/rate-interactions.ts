...
export function getRatePointerValue(root: HTMLElement, clientX: number, step: number, count: number, direction: RateDirection) {
  ...
  for (const item of items) {
    ...
    if (clientX >= rect.left && clientX <= rect.right) {
      ...
      return getRateValueFromPointer(index, direction === 'rtl' ? 1 - visualRatio : visualRatio, step, count)
    }
