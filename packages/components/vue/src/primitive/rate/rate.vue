...
}>(), { count: 5, step: 1, disabled: false, readOnly: false, allowClear: true, direction: 'ltr', size: 'default' })
...
const options = {
  get value() { return props.value }, get defaultValue() { return props.defaultValue },
  get count() { return props.count }, get step() { return props.step },
  get disabled() { return props.disabled }, get readOnly() { return props.readOnly },
  get allowClear() { return props.allowClear }, get direction() { return props.direction },
...
const snapshot = computed(() => { void storeSnapshot.value; return controller.getSnapshot() })

function pointerValue(event: PointerEvent) {
  if (!root.value) return 0
  return getRatePointerValue(root.value, event.clientX, snapshot.value.step, snapshot.value.count, snapshot.value.direction)
}
function handlePointerDown(event: PointerEvent) {
  if (event.defaultPrevented || snapshot.value.disabled || snapshot.value.readOnly || !root.value) return
...
function handleKeydown(event: KeyboardEvent) {
  ...
  if (event.key === 'Home' || event.key === 'End') { event.preventDefault(); controller.setValue(event.key === 'Home' ? 0 : snapshot.value.count, { commit: true }) }
...
function itemState(index: number, layer: 'empty' | 'filled') {
  ...
  return { index, layer, fill, fillPercent: fill * 100, full: fill === 1, partial: fill > 0 && fill < 1, empty: fill === 0, previewing: snapshot.value.previewValue !== null, disabled: snapshot.value.disabled, readOnly: snapshot.value.readOnly }
}
...
<template>
  <div v-bind="attrs" ref="root" role="slider" :tabindex="snapshot.disabled ? undefined : 0" :dir="snapshot.direction" :aria-valuemin="0" :aria-valuemax="snapshot.count" :aria-valuenow="snapshot.value" :aria-valuetext="getValueText?.(snapshot.value, snapshot.count) ?? `${snapshot.value} out of ${snapshot.count}`" :aria-disabled="snapshot.disabled || undefined" :aria-readonly="snapshot.readOnly || undefined" :data-disabled="snapshot.disabled ? 'true' : undefined" :data-readonly="String(snapshot.readOnly)" :class="cn(rateRootClassName({ size: props.size }), attrs.class as string | undefined)" @pointerdown="handlePointerDown" @pointermove="handlePointerMove" @pointerup="handlePointerUp" @pointercancel="controller.cancelInteraction" @pointerleave="controller.clearPreview" @pointerout="handlePointerOut" @keydown="handleKeydown">
    <span v-for="index in snapshot.count" :key="index" :data-rate-item="index - 1" :class="rateItemClassName">
      <span :class="rateEmptyContentClassName"><slot v-bind="itemState(index - 1, 'empty')"><StarIcon aria-hidden="true" /></slot></span>
      <span :class="rateFilledContentClassName" :style="{ clipPath: clipPath(index - 1) }"><slot v-bind="itemState(index - 1, 'filled')"><StarIcon aria-hidden="true" /></slot></span>
    </span>
  </div>
</template>
