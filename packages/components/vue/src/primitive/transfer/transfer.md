...
使用 `sourceHeader`、`sourceBody`、`sourceFooter`、`targetHeader`、`targetBody`、`targetFooter`、`actions` 和 `item` slots。Panel slot 的 `api` 提供 `items`、`checkedKeys`、`setCheckedKeys`、`isChecked` 与同一个 `controller`；actions slot 同时提供 `controller` 和响应式 `snapshot`。Tree/DataGrid 只需把选择状态绑定到 panel API，移动仍由 Transfer controller 负责。
