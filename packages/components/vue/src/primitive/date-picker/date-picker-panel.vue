...
import { CalendarCell as CalendarCellView, CalendarGrid, CalendarRoot, CalendarWeekHeader } from '../calendar/calendar'
...
const props = defineProps<{ class?: string; panelViewDate?: CalendarDate; range?: boolean }>()
...
function selectCell(cell: CalendarCell) {
  ...
  context.select(normalizeDatePickerValue(nextViewDate, context.picker, context.weekStartsOn) as never)
}

function disabledDate(date: CalendarDate) {
  return rangePicker ? rangePicker.disabledDate?.(date, rangePicker.activePart.value) ?? false : datePicker?.disabledDate?.(date) ?? false
}
    ...
    <slot>
      <DatePickerHeader>
        <div :class="datePickerHeaderSideClassName">
          <DatePickerHeaderButton :action="owner.panel.value === 'date' ? 'previous-year' : 'previous-panel'" />
        ...
        <div :class="datePickerHeaderSideClassName">
          ...
          <DatePickerHeaderButton :action="owner.panel.value === 'date' ? 'next-year' : 'next-panel'" />
        </div>
      </DatePickerHeader>
      <CalendarWeekHeader v-if="owner.panel.value === 'date'" :class="datePickerWeekHeaderClassName" />
