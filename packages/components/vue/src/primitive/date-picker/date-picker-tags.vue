...
const props = withDefaults(defineProps<{ class?: string; maxTagCount?: number }>(), { maxTagCount: 1 })
...
const overflowCount = computed(() => Math.max(context.calendarValues.value.length - props.maxTagCount, 0))
...
<template>
  <div data-slot="date-picker-tags" :class="className">
    <span v-for="value in visibleValues" :key="label(value)" data-slot="date-picker-tag" :class="datePickerTagClassName">
      {{ label(value) }}
      <button
        type="button"
    ...
    <span v-if="overflowCount > 0" data-slot="date-picker-tag-overflow" :class="datePickerTagOverflowClassName">+{{ overflowCount }}</span>
  </div>
</template>
