...
export function useUploadMd5(id: MaybeRefOrGetter<UploadId>) {
  ...
  const unsubscribe = feature ? upload.subscribeFeatureItem('file-md5', toValue(id), () => { state.value = feature.getState(toValue(id)) }) : undefined
  onScopeDispose(() => unsubscribe?.())
  return { available: Boolean(feature), state, calculate: () => feature?.calculate(toValue(id)), cancel: () => feature?.cancel(toValue(id)) }
}
...
export function useUploadParts<TResponse = unknown>(id: MaybeRefOrGetter<UploadId>) {
  ...
  const unsubscribe = upload.subscribeFeatureItem('upload', toValue(id), () => { parts.value = feature?.getParts?.(toValue(id)) ?? [] })
  onScopeDispose(unsubscribe)
  return parts
}
...
export function useUploadProgress(id: MaybeRefOrGetter<UploadId>, options: { md5Weight?: number } = {}) {
  const { upload } = useUploadContext()
  ...
  return computed(() => {
    ...
    else if (status === 'error' && md5.available && md5.state.value?.status !== 'success') percent = md5Percent * weight
