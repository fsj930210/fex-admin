...
export interface UseDroppableOptions<TData extends Record<string, unknown> = Record<string, unknown>> {
  ...
  getPosition?: (args: { element: HTMLElement; pointer: Point; source: Record<string, unknown> }) => DropPosition | null
...
export function useDroppable<TData extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const acceptsSource = (source: Record<string, unknown>) => {
    ...
    return (acceptList.length === 0 || acceptList.includes(String(source.type))) && (options.canDrop?.(source) ?? true)
  }
  ...
  function setTarget(element: HTMLElement | null) {
    ...
    if (!element || options.disabled) { cleanupTarget = undefined; return }
    ...
    cleanupTarget = registerDndDropTarget({
      ...
      ...(options.positions ? { positions: options.positions } : options.edges ? { edges: options.edges } : {}),
      getPosition: ({ element: target, pointer, source }) => options.getPosition?.({ element: target, pointer, source })
        ?? getDropPositionAtPoint(target, pointer, positions),
      onDragEnter: (args) => {
        over.value = true; canDrop.value = true; position.value = args.position
        options.onDragEnter?.({ ...args, target: args.target as TData & { id: string } })
      },
      onDrag: (args) => {
        canDrop.value = true; position.value = args.position
        options.onDrag?.({ ...args, target: args.target as TData & { id: string } })
      },
      onDragLeave: () => {
        over.value = false; canDrop.value = false; position.value = null; options.onDragLeave?.()
      },
      onDrop: (args) => {
        over.value = false; canDrop.value = false; position.value = null
  ...
  const edge = computed<DropEdge | null>(() => position.value === 'inside' ? null : position.value)
