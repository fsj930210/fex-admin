...
export class DataGridColumnOverlay {
  ...
  protected readonly columnWidth = computed(
    () => `${getDataGridColumnSize(this.renderHeader().column)}px`,
  )
  protected readonly rows = computed(() => this.renderTable().getRowModel().rows)

  protected cell(
    row: ReturnType<AngularDataGridTable<TableFeatures, RowData>['getRowModel']>['rows'][number],
  ) {
    return getDataGridRenderedCells(row).find(
