...
export function useUploadFeature<TApi>(id: string) { return useUploadContext().upload.getFeature<TApi>(id) }

export function useUploadMd5(id: UploadId) {
  const { upload } = useUploadContext()
  const feature = upload.getFeature<FileMd5FeatureApi>('file-md5')
  const state = useSyncExternalStore((listener) => upload.hasFeature('file-md5') ? upload.subscribeFeatureItem('file-md5', id, listener) : () => {}, () => feature?.getState(id), () => feature?.getState(id))
...
export function useUploadParts<TResponse = unknown>(id: UploadId) {
  ...
  return useSyncExternalStore((listener) => upload.subscribeFeatureItem('upload', id, listener), () => feature?.getParts?.(id) ?? emptyParts as readonly UploadPart<TResponse>[], () => emptyParts as readonly UploadPart<TResponse>[])
}

export function useUploadPreview(id: UploadId) { return useUploadFeature<PreviewFeatureApi>('preview')?.getUrl(id) }
export function useUploadDragging() {
  const feature = useUploadFeature<DropFeatureApi>('drop')
  return useSyncExternalStore(feature?.subscribe ?? (() => () => {}), feature?.getDragging ?? (() => false), () => false)
}
