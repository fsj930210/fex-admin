...
function getResizeDirection(
  ...
  return (
    checks.find(([matched, direction]) => matched && isAllowed(direction, directions))?.[1] ?? null
  )
}
...
export function useResize<T extends HTMLElement = HTMLElement>(options: ResizeOptions<T> = {}) {
  ...
  const startResize = useCallback(
    ...
    [targetNode, handlePointerMove, handlePointerUp],
  )
  ...
  const handleEdgePointerDown = useCallback(
    (event: PointerEvent) => {
      ...
      const {
        ...
        enableEdgeResize,
        edgeSize = 12,
        cornerSize = Math.max(edgeSize * 2, 24),
        directions,
      } = optionsRef.current
