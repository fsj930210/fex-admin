...
<div {...rest} class={cn(uploadDropzoneClassName(), className)} role="button" tabindex={upload.getOptions().disabled ? undefined : 0} aria-disabled={upload.getOptions().disabled} aria-invalid={invalid()} data-dragging={dragging || undefined} data-disabled={upload.getOptions().disabled || undefined} data-invalid={invalid() || undefined} ondragenter={(event) => { if (drop) { event.preventDefault(); drop.dragEnter() } }} ondragover={(event) => { if (drop) event.preventDefault() }} ondragleave={() => drop?.dragLeave()} ondrop={(event) => { if (drop) { event.preventDefault(); void getDroppedFiles(event.dataTransfer!).then((files) => drop.drop(files)) } }} onpaste={(event) => { const files = [...(event.clipboardData?.files ?? [])]; if (paste && files.length) { event.preventDefault(); void paste.paste(files) } }}>{@render children?.()}</div>
