...
import type { DatePickerContextValue, DatePickerSelectionValue, RangePickerContextValue } from './context'
...
export function useDatePicker<TValue extends CalendarValue = CalendarValue>(options: UseDatePickerOptions<TValue>): DatePickerContextValue<TValue> {
  const picker = options.picker ?? 'date'
  const multiple = options.multiple ?? false
  const needConfirm = options.needConfirm ?? multiple
  let localValue = $state<DatePickerSelectionValue<TValue>>(options.defaultValue ?? (multiple ? [] : null))
  ...
  function resetPanel() { const current = value(); const selected = isValueArray(current) ? current[0] : current; pendingValue = current; panel = getDefaultPanelByPicker(picker); viewDate = selected ? getCalendarValueDate(selected) : getCalendarToday() }
  function setOpen(next: boolean) { const wasOpen = open(); if (options.open === undefined) localOpen = next; if (next && !wasOpen) resetPanel(); if (!next) panel = getDefaultPanelByPicker(picker); if (wasOpen !== next) options.onOpenChange?.(next) }
  function commit(next: DatePickerSelectionValue<TValue>) { if (options.value === undefined) localValue = next; options.onChange?.(next) }
  ...
  function select(next: TValue) {
    if (multiple) { const values = isValueArray(activeValue()) ? activeValue() as readonly TValue[] : []; const exists = values.some((item) => isSameCalendarValue(item, next)); const result = exists ? values.filter((item) => !isSameCalendarValue(item, next)) : [...values, next]; if (needConfirm) pendingValue = result; else commit(result); return }
...
export function useRangePicker<TValue extends CalendarValue = CalendarValue>(options: UseRangePickerOptions<TValue>): RangePickerContextValue<TValue> {
  ...
  function resetPanel(part?: 'start' | 'end') { const current = value(); const selected = current.start ?? current.end; pendingValue = current; panel = getDefaultPanelByPicker(picker); viewDate = selected ? getCalendarValueDate(selected) : getCalendarToday(); activePart = part ?? (current.start ? 'end' : 'start'); hoverValue = null }
  function setOpen(next: boolean, part?: 'start' | 'end') { const wasOpen = open(); if (next && !wasOpen) resetPanel(part); else if (next && part) activePart = part; if (options.open === undefined) localOpen = next; if (wasOpen !== next) options.onOpenChange?.(next) }
  function close() { setOpen(false) }
  function commit(next: CalendarRange<TValue>) { if (options.value === undefined) localValue = next; pendingValue = next; options.onChange?.(next) }
  function select(nextValue: TValue) { const part = activePart; const nextRange = createNextRangeValue(rangeValue(), nextValue, part, options.order ?? true); hoverValue = null; if (needConfirm) pendingValue = nextRange; else commit(nextRange); const nextActivePart = getNextRangeActivePart(nextRange); if (nextActivePart) { activePart = nextActivePart; return }; if (!needConfirm) close() }
  function clear() { const next: CalendarRange<TValue> = {}; pendingValue = next; hoverValue = null; commit(next) }
  function confirm() { commit(pendingValue); close() }
  function cancel() { pendingValue = value(); close() }
