...
export interface SearchFeatureApi<TNode extends TreeNodeData> {
  getSubtree(options: TreeSearchSubtreeOptions<TNode>): readonly TNode[]
}
  ...
  return {
    ...
    feature: {
      ...
      setup: (context) =>
        ({
          getSubtree: (options) =>
            getSearchSubtree(context.getSnapshot().treeData, context.getFieldNames(), options),
