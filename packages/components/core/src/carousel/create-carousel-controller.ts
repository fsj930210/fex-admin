...
import type { CarouselController, CarouselControllerConfig, CarouselSnapshot } from './types'
...
export function createCarouselController(
  config: CarouselControllerConfig = {},
): CarouselController {
  ...
  function sync() {
    store.setSnapshot(readSnapshot())
  }
  function emitSelect() {
    sync()
    getConfig().onSelect?.(store.getSnapshot())
  }
  function emitSettle() {
    sync()
    getConfig().onSettle?.(store.getSnapshot())
  }
  ...
  function getPlugins(): EmblaPluginType[] {
    ...
    return [
      ...(currentConfig.autoplay
        ? [Autoplay(currentConfig.autoplay === true ? undefined : currentConfig.autoplay)]
        : []),
      ...plugins,
    ]
  }
  ...
  return {
    ...
    mount(viewport) {
      ...
      embla
        ...
        .on('settle', emitSettle)
        .on('reInit', sync)
        .on('slidesInView', sync)
        .on('autoplay:play', sync)
        .on('autoplay:stop', sync)
      sync()
    },
    scrollTo(index, jump) {
