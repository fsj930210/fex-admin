...
/* eslint-disable unicorn/require-post-message-target-origin -- DedicatedWorkerGlobalScope postMessage has no targetOrigin parameter. */
  ...
  try {
    for (let start = 0; start < file.size; start += chunkSize) {
      ...
      self.postMessage({
        id,
        type: 'progress',
        loaded: Math.min(start + chunkSize, file.size),
        total: file.size,
      })
    }
    // oxlint-disable-next-line unicorn/require-post-message-target-origin -- Worker postMessage has no target origin.
