...
type RenderTable<TFeatures extends TableFeatures, TData extends RowData> = SolidDataGridTable<
  TFeatures,
  TData
> &
...
function layoutStyle(
  style: ReturnType<typeof getDataGridColumnLayout>['style'],
): JSX.CSSProperties {
  return {
  ...
  return (
    ...
    >
      <div
        ...
        class={cn(dataGridViewportClassName, local.class?.viewport)}
          ...
          <thead class={cn(dataGridHeaderClassName, local.class?.header)}>
            <For each={headerGroups()}>
              {(headerGroup) => (
                <tr
                  class={cn(
                    dataGridHeaderRowClassName({ bordered: local.border ?? false }),
            ...
            <Show
              ...
              fallback={
                <tr>
                  <td
                    colSpan={Math.max(1, getDataGridVisibleLeafColumnCount(renderTable))}
                    class={cn(dataGridEmptyClassName, local.class?.empty)}
                  >
      ...
      <Show when={local.loading}>
        <div class={cn(dataGridLoadingClassName, local.class?.loading)}>
