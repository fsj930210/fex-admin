...
export interface DatePickerFooterProps extends ParentProps<JSX.HTMLAttributes<HTMLDivElement>> {
  children?: JSX.Element | ((actions: { close: () => void; clear: () => void; confirm: () => void; cancel: () => void }) => JSX.Element)
}

export function DatePickerFooter(props: DatePickerFooterProps) {
  const [local, rest] = splitProps(props, ['class', 'children'])
  const picker = useFooterOwner()
  const actions = { close: picker.close, clear: picker.clear, confirm: picker.confirm, cancel: picker.cancel }
  return (
...
export function DatePickerConfirm(props: ButtonProps) {
  ...
  return <Button {...rest} data-slot="date-picker-confirm" class={cn(datePickerConfirmClassName, local.class)} onClick={(event) => { local.onClick?.(event); if (!event.defaultPrevented) picker.confirm() }} />
}

export function DatePickerCancel(props: ButtonProps) {
  const picker = useFooterOwner()
  const [local, rest] = splitProps(props, ['class', 'onClick'])
  return <Button {...rest} data-slot="date-picker-cancel" class={cn(datePickerCancelClassName, local.class)} onClick={(event) => { local.onClick?.(event); if (!event.defaultPrevented) picker.cancel() }} />
}

export function DatePickerToday(props: ButtonProps) {
  const picker = useFooterOwner()
  const [local, rest] = splitProps(props, ['class', 'onClick'])
  return <Button {...rest} data-slot="date-picker-today" class={cn(datePickerTodayClassName, local.class)} onClick={(event) => { local.onClick?.(event); if (!event.defaultPrevented) { picker.setViewDate(getCalendarToday()); picker.setPanel(getDefaultPanelByPicker(picker.picker)) } }} />
}

export function DatePickerPreset(props: ButtonProps) {
  const [local, rest] = splitProps(props, ['class'])
  return <Button {...rest} data-slot="date-picker-preset" class={cn(datePickerPresetClassName, local.class)} />
}
