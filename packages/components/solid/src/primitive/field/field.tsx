...
import { createComponent, createContext, createUniqueId, For, Show, splitProps, useContext, type JSX, type ParentProps } from 'solid-js'
import { useFormContext } from '../form/form'

export interface FieldState { disabled: boolean; invalid: boolean; readOnly: boolean; required: boolean }
export interface FieldControlBinding { props: { id: string; disabled?: true | undefined; readOnly?: true | undefined; 'aria-required'?: true | undefined; 'aria-invalid'?: true | undefined; 'aria-describedby'?: string | undefined; 'aria-errormessage'?: string | undefined; 'data-field-name'?: string | undefined }; state: FieldState }
...
export function Field(props: { name: string; children: (field: any) => JSX.Element; [key: string]: unknown }) {
  const form = useFormContext()
  const fieldProps = {
    ...props,
    children: (field: any) => {
      const fieldName = String(field.name ?? props.name)
      return withRenderingFieldName(fieldName, () => <FormFieldNameContext.Provider value={fieldName}>{props.children(field)}</FormFieldNameContext.Provider>)
    },
  }
...
export function FieldControl(props: { children: (binding: FieldControlBinding) => JSX.Element }) {
  ...
  return props.children({ props: { id: context.controlId, disabled: context.disabled || undefined, readOnly: context.readOnly || undefined, 'aria-required': context.required || undefined, 'aria-invalid': context.invalid || undefined, 'aria-describedby': describedBy(), 'aria-errormessage': context.invalid && context.hasError ? context.errorId : undefined, 'data-field-name': fieldName }, state: context })
}
...
export function FieldGroup(props: JSX.HTMLAttributes<HTMLDivElement> & FieldGroupStyleProps) { const [local, rest] = splitProps(props, ['class', 'orientation']); return <div {...rest} role={props.role ?? 'group'} data-slot="field-group" data-orientation={local.orientation} class={cn(fieldGroupClassName({ orientation: local.orientation }), local.class)} /> }
