...
export class UploadService {
  ...
  async savePart(stream: Readable, uploadId: string, partNumber: number): Promise<PartResult> {
  ...
  async completeMultipart(uploadId: string, body: CompleteMultipartDto): Promise<UploadResult> {
    const directory = this.getPartsPath(uploadId)
    const expected = Array.from({ length: body.partCount }, (_, index) => index)
    const names = await readdir(directory).catch(() => [])
  ...
  private getRecordPath(md5: string) {
    if (!/^[a-f\d]{32}$/i.test(md5)) throw new BadRequestException('A valid MD5 is required.')
    return join(this.recordsDirectory, `${md5}.json`)
  }

  private assertUploadId(uploadId: string) {
    if (!/^[a-z\d-]+$/i.test(uploadId)) throw new BadRequestException('Invalid upload ID.')
  }
  ...
  private async mergeParts(directory: string, indexes: readonly number[], target: string) {
    await mkdir(dirname(target), { recursive: true })
    async function* readParts() {
      for (const index of indexes) yield* createReadStream(join(directory, String(index)))
    }
