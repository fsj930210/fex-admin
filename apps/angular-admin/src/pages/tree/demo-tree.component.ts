...
export
@Component({
  selector: 'fex-demo-tree',
  standalone: true,
  imports: [TreeRoot, TreeViewport, TreeVirtualViewport, DemoTreeRowComponent],
  templateUrl: './demo-tree.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
class DemoTreeComponent {
  readonly options = input.required<TreeOptions<DepartmentNode>>()
  readonly controller = input<TreeController<DepartmentNode> | undefined>()
  ...
  readonly viewport = viewChild(TreeVirtualViewport)
  scrollToKey(
    key: TreeKey,
    options?: { align?: 'auto' | 'start' | 'center' | 'end'; reveal?: boolean },
  ) {
    return this.viewport()?.scrollToKey(key, options) ?? false
  }
}
