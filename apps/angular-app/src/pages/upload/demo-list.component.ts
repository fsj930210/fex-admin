...
@Component({
  ...
  imports: [
    ...
    UploadItemPreview,
    UploadItemProgress,
    MultipartDetailsComponent,
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './demo-list.component.html',
})
export class DemoUploadListComponent {
  readonly showMultipart = input(false, { transform: booleanAttribute })
  protected readonly context = inject(UploadContext)
  protected message(error: unknown) {
    return error instanceof Error ? error.message : String(error)
  }
  protected instant(response: unknown) {
    return (
