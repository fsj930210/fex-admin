import { createCalendarDate, getCalendarValueDate, type CalendarRange, type CalendarValue } from '@fex/components-core/calendar'
import { isAfterDate, isBeforeDate } from '@fex/components-core/date/utils'
import { DatePickerCancel, DatePickerConfirm, DatePickerFooter, DatePickerToday } from '@fex/components-solid/primitive/date-picker'
...
export function BasicDemos() {
  ...
  return (
    <>
      ...
      <Section title="受控与非受控" description="value/onChange 管理受控值，defaultValue 提供非受控初始值。">
        <div><DemoDatePicker value={controlled()} onChange={(next) => setControlled(Array.isArray(next) ? null : next)} /><ValuePreview value={controlled()} /></div>
        <DemoDatePicker defaultValue={createCalendarDate(2026, 8, 1)} />
      </Section>
      <Section title="禁用日期" description="minDate/maxDate 限定固定区间，disabledDate 承载业务规则。">
        <DemoDatePicker minDate={minDate} maxDate={maxDate} />
        <DemoDatePicker disabledDate={(date) => isBeforeDate(date, today) || date.dayOfWeek === 6} />
        <DemoDatePicker disabled />
      </Section>
      <Section title="多选" description="multiple 使用 CalendarValue 数组，默认需要确认，不会选中后立刻关闭。">
        <div>
          <DemoDatePicker multiple value={multiple()} onChange={setMultiple} footer={<DatePickerFooter><DatePickerToday>今天</DatePickerToday><DatePickerCancel>取消</DatePickerCancel><DatePickerConfirm>确定</DatePickerConfirm></DatePickerFooter>} />
          <ValuePreview value={multiple()} />
        </div>
      </Section>
      <Section title="RangePicker" description="范围选择使用专门入口，双面板复用 Calendar range state。">
        <div><DemoRangePicker value={range()} onChange={setRange} /><RangePreview value={range()} /></div>
      </Section>
      <Section title="动态范围禁用" description="选择一端后通过 disabledDate 限制另一端，只允许 7 天窗口。">
        <div><DemoRangePicker value={dynamicRange()} onChange={setDynamicRange} disabledDate={dynamicDisabled} /><RangePreview value={dynamicRange()} /></div>
      </Section>
    </>
  )
}
