...
interface Props<T extends TreeNodeData> extends TreeOptions<T> {
  ...
  indent?: number
  searchKeyword?: string
  titleRender?: (context: TitleContext<T>) => JSX.Element
  class?: string
  itemClass?: string
  ref?: (handle: TreeVirtualViewportHandle) => void
}
function Row<T extends TreeNodeData>(props: {
  tree: TreeController<T>
  item: TreeVisibleItem<T>
  checkable: boolean
  keyword: string
  titleField: string
  titleRender?: Props<T>['titleRender']
  itemClass?: string
}) {
  return (
    <TreeItem<T> itemKey={props.item.key} {...(props.itemClass === undefined ? {} : { class: props.itemClass })}>
      {({ item, itemProps, checkedState, loadState, actions }) => (
        <div {...itemProps}>
          ...
          <Show when={props.checkable && props.tree.hasFeature('check')}>
            <Checkbox
          ...
          <TreeTitle>
            {props.titleRender?.({
              item,
              tree: props.tree,
              isSearching: Boolean(props.keyword),
              searchKeyword: props.keyword,
            }) ?? String(item.node[props.titleField] ?? item.key)}
          </TreeTitle>
        </div>
      )}
    </TreeItem>
...
export function DemoTree<T extends TreeNodeData>(props: Props<T>) {
  const options = (): TreeOptions<T> => ({
            ...
            <TreeViewport<T>>
              {(item) => (
                <Row
                  ...
                  {...(props.titleRender === undefined ? {} : { titleRender: props.titleRender })}
                  {...(props.itemClass === undefined ? {} : { itemClass: props.itemClass })}
                />
              )}
            </TreeViewport>
          }
        >
          <TreeVirtualViewport<T>
