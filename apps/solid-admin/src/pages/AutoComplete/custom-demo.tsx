import { AutoCompleteContent, AutoCompleteList, AutoCompleteRoot, AutoCompleteTrigger } from '@fex/components-solid/primitive/auto-complete'
  ...
  return (
    <div class="grid gap-space-xl md:grid-cols-2">
      <Card title="Custom items and disabled suggestion" description="Original backend items drive richer rows; Alex is disabled.">
        <AutoCompleteRoot items={users} fieldNames={fieldNames}>
          ...
          <AutoCompleteContent>
            ...
            <AutoCompleteList item={(item, state) => {
              const user = item as unknown as UserSuggestion
              return <div><div class="font-medium">{user.name}</div><div class="text-xs text-muted-foreground">{user.department} · {user.email}{state.disabled ? ' · unavailable' : ''}</div></div>
            }} />
          </AutoCompleteContent>
        </AutoCompleteRoot>
      </Card>
      <Card title="Validation and native props" description="Input validation styles and native form attributes are preserved.">
        <AutoCompleteRoot items={users} fieldNames={fieldNames}>
          <AutoCompleteTrigger name="reviewer" required aria-describedby="reviewer-error-solid" placeholder="Required reviewer" clearable invalid status="error" />
          <AutoCompleteContent />
        </AutoCompleteRoot>
        <p id="reviewer-error-solid" class="mt-space-sm text-xs text-danger">Choose a reviewer or enter a new name.</p>
      </Card>
    </div>
  )
}
