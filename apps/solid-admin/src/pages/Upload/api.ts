...
async function requestJson<T>(url: string, init?: RequestInit) {
  ...
  const value = JSON.parse(await response.text()) as { code: number; message: string; data: T }
...
export function uploadBody<T>(
  ...
  return new Promise<T>((resolve, reject) => {
    ...
    request.onload = () => {
      ...
      try {
        const value = JSON.parse(request.responseText) as { code: number; message: string; data: T }
        if (request.status >= 200 && request.status < 300 && value.code >= 200 && value.code < 300)
          resolve(value.data)
