...
import { ListboxItem as PrimitiveListboxItem, ListboxRoot as PrimitiveListboxRoot } from '@fex/components-solid/primitive/listbox'
import { ListboxGroup, ListboxGroupLabel, ListboxItem, ListboxRoot } from '@fex/components-solid/ui/listbox'
...
export function ListboxPage(): JSX.Element {
  ...
  const [selectedTricks, setSelectedTricks] = createSignal<(string | number)[]>(['kickflip', 'heelflip'])
      ...
      <div class="mx-auto w-full max-w-5xl space-y-space-xl">
        ...
        <div class="space-y-space-xl">
          <Card title="Primitive" description="Headless root and item parts without default item layout.">
            <PrimitiveListboxRoot items={tricks} defaultValue="kickflip" class="flex max-w-xl flex-col gap-2">
          ...
          <Card title="Single" description="Controlled single selection uses onChange.">
            ...
            >
              <For each={tricks.slice(0, 4)}>
                {(trick) => <ListboxItem value={trick.value} title={trick.title} description={trick.description} />}
              </For>
            </ListboxRoot>
          </Card>
          ...
          <Card title="Horizontal" description="Horizontal orientation lays items out in rows.">
            <ListboxRoot items={tricks} defaultValue="tre-flip" orientation="horizontal">
              <For each={tricks.slice(0, 4)}>
                {(trick) => <ListboxItem class="w-56" value={trick.value} title={trick.title} description={trick.description} />}
              </For>
            </ListboxRoot>
          </Card>

          <Card title="Grouped Items" description="Group related options with the same controller.">
            <ListboxRoot multiple items={tricks} defaultValue={['kickflip', 'heelflip']} class="mx-auto max-w-xl">
              <ListboxGroup>
