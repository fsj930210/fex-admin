...
import { Checkbox, CheckboxGroup, type CheckboxCheckedState } from '@fex/components-solid/ui/checkbox'
...
function DemoSection(props: { title: string; description: string; children: JSX.Element }) {
  return <Card title={props.title} description={props.description}><div class="grid min-w-0 gap-space-md">{props.children}</div></Card>
}
      ...
      <div class="mx-auto w-full max-w-5xl space-y-space-xl">
        <header class="space-y-space-xl">
          ...
          <div>
            ...
            <p class="mt-space-md max-w-2xl text-sm leading-6 text-muted-foreground">Boolean checks, grouped layouts, indeterminate state, disabled state, and aria-invalid styling.</p>
          </div>
        </header>
        <div class="space-y-space-xl">
          <DemoSection title="Primitive" description="Self-owned checkbox primitive exported as low-level parts."><CheckboxRoot defaultChecked aria-label="Primitive checkbox" /></DemoSection>
          <DemoSection title="Basic" description="Default UI wrapper with native data and aria attributes.">
            ...
            <div class="inline-flex items-center gap-2 text-sm"><Checkbox aria-invalid /><span>Invalid style from aria-invalid</span></div>
          </DemoSection>
          <DemoSection title="Controlled" description="Controlled state uses checked and onCheckedChange.">
            <div class="inline-flex items-center gap-2 text-sm"><Checkbox checked={controlled()} onCheckedChange={setControlled} /><span>Controlled checkbox</span></div>
          ...
          <DemoSection title="Indeterminate" description="Compose check-all behavior from normal events.">
            <div class="inline-flex items-center gap-2 text-sm">
              <Checkbox checked={partialChecked() ? 'indeterminate' : allChecked()} onCheckedChange={(checked) => setPartialValue(checked === true ? optionValues : [])} />
              <span>Check all permissions</span>
            </div>
            <CheckboxGroup>
              <For each={options}>{(option) => (
                <div class="inline-flex items-center gap-2 text-sm">
                  <Checkbox checked={partialValue().includes(option.value)} onCheckedChange={(checked) => setPartialValue((current) => nextValues(current, option.value, checked))} />
          ...
          <DemoSection title="Group" description="CheckboxGroup is a layout wrapper; value arrays stay in the caller.">
            <CheckboxGroup orientation="horizontal">
              <For each={options}>{(option) => (
                <div class="inline-flex items-center gap-2 text-sm">
                  <Checkbox checked={permissions().includes(option.value)} onCheckedChange={(checked) => setPermissions((current) => nextValues(current, option.value, checked))} />
                  <span>{option.label}</span>
                </div>
              )}</For>
            </CheckboxGroup>
