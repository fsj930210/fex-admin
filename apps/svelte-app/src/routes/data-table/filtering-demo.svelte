...
<DemoSection title="Column filtering" description="meta.filterFn opts a column into local filtering. Missing filterFn means remote, while onColumnFiltersChange still receives the complete filter list.">
  <div class="grid gap-space-lg xl:grid-cols-3">{#each grids as item (item.title)}<section class="space-y-space-sm"><h3 class="text-sm font-medium text-foreground">{item.title}</h3><div class="flex flex-wrap gap-space-sm">{#each item.table.getAllLeafColumns() as column (column.id)}<InputRoot class="h-8 w-auto min-w-0 flex-1"><InputControl placeholder={`Filter ${column.id}`} oninput={event => column.setFilterValue(event.currentTarget.value)} /></InputRoot>{/each}</div><DataTable table={item.table} /><ReactiveTableText snapshot={item.table.dataTableSnapshot} text={revision => remoteText(item, revision)} /></section>{/each}</div>
</DemoSection>
