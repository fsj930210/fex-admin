...
const headlessEntries = computed(() =>
  headlessMenu.nodeItems.value.filter((entry) =>
    entry.keyPath
      .slice(0, -1)
      .every((key) => !headlessEntryFor(key) || headlessMenu.currentExpandKeys.value.includes(key)),
  ),
)
</script>
    ...
    <div class="mx-auto w-full max-w-5xl space-y-space-xl">
      ...
      <div class="grid gap-space-xl lg:grid-cols-2">
        ...
        >
          <Menu
            :items="menuItems"
            :default-expand-keys="['system', 'content']"
            :default-selected-keys="['users', 'articles']"
            select-multiple
          />
        </Card>
        <Card title="Suffix" description="Right-side content can be provided by data.">
          <Menu
            :items="menuItems"
            :default-expand-keys="['system', 'content']"
            :default-selected-keys="['comments']"
          />
        </Card>
          ...
          <Menu :items="menuItems" :default-expand-keys="['system']">
            <template #item="{ info }">
              ...
              <span class="min-w-0 flex-1 truncate"
                >{{ info.item.label
                }}<span v-if="info.selected" class="ml-2 text-xs text-primary">selected</span></span
              >
              <span v-if="info.hasChildren" class="text-xs text-muted-foreground">{{
                info.expanded ? 'open' : 'closed'
              }}</span>
            </template>
          </Menu>
        </Card>
        <Card
          ...
          description="useMenu exposes state and events for custom DOM."
        >
          <nav class="space-y-1" role="menu">
            <button
              v-for="entry in headlessEntries"
              :key="entry.key"
              type="button"
              role="menuitem"
              ...
              :data-selected="headlessMenu.getItemInfo(entry).selected ? 'true' : 'false'"
              @click="headlessMenu.clickItem(headlessMenu.getItemInfo(entry))"
            >
              <span class="min-w-0 flex-1 truncate">{{
                headlessMenu.getItemInfo(entry).item.label
              }}</span>
              <span
                v-if="headlessMenu.getItemInfo(entry).hasChildren"
                class="inline-flex size-4 shrink-0 items-center justify-center text-muted-foreground"
                ><MinusIcon
                  v-if="headlessMenu.getItemInfo(entry).expanded"
                  class="size-4" /><PlusIcon v-else class="size-4"
              /></span>
            </button>
          </nav>
        </Card>
