    ...
    <div class="mx-auto w-full max-w-5xl space-y-space-xl">
      <header class="space-y-space-md">
        <RouterLink class="text-sm text-muted-foreground hover:text-foreground" to="/"
          >返回首页</RouterLink
        >
        <h1 class="text-2xl font-semibold text-foreground">Textarea</h1>
      ...
      <Card title="States" description="常用多行输入状态。">
        <div class="grid gap-space-lg">
          <Textarea placeholder="记录本次处理说明" /><Textarea
            default-value="只读多行内容"
            disabled
          /><Textarea aria-invalid="true" placeholder="校验失败" />
        </div>
      </Card>
    </div>
  </main>
