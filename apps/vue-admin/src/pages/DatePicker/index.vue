...
function setRange(next: unknown) { range.value = next as CalendarRange<CalendarValue> }
function setDynamicRange(next: unknown) { dynamicRange.value = next as CalendarRange<CalendarValue> }
function setSubmitted(next: unknown) { submitted.value = next as CalendarRange<CalendarValue> }
function setControlledValue(next: unknown) { setDate(next as CalendarValue | readonly CalendarValue[] | null, controlled) }
function setPanelValue(next: unknown) { setDate(next as CalendarValue | readonly CalendarValue[] | null, panelValue) }
function setMultipleValue(next: unknown) { setMultiple(next as CalendarValue | readonly CalendarValue[] | null) }
    ...
    <div class="mx-auto w-full max-w-5xl space-y-space-xl">
      <header class="space-y-space-xl">
        ...
        <div>
          ...
          <p class="mt-space-md max-w-2xl text-sm leading-6 text-muted-foreground">DatePicker primitive 组合 Input、Popover 和 Calendar，示例覆盖单选、多选、范围、禁用、面板切换和自定义渲染。</p>
        </div>
      </header>
      <div class="space-y-space-xl">
        <Card title="基本用法" description="非受控 DatePicker 选择后立即提交并关闭 Popover。"><div :class="demoContentClass"><DemoDatePicker :default-value="today" placeholder="请选择日期" /></div></Card>
        <Card title="受控与非受控" description="value/onChange 管理受控值，defaultValue 提供非受控初始值。"><div :class="demoContentClass"><div><DemoDatePicker :value="controlled" @change="setControlledValue" /><p class="mt-space-sm w-full text-xs text-muted-foreground">当前值：{{ controlled ? getCalendarValueKey(controlled) : '未选择' }}</p></div><DemoDatePicker :default-value="createCalendarDate(2026, 8, 1)" /></div></Card>
        <Card title="禁用日期" description="minDate/maxDate 限定固定区间，disabledDate 承载业务规则。"><div :class="demoContentClass"><DemoDatePicker :min-date="today.subtract({ days: 18 })" :max-date="today.add({ days: 25 })" /><DemoDatePicker :disabled-date="disabledWeekend" /><DemoDatePicker disabled /></div></Card>
        <Card title="多选" description="multiple 使用 CalendarValue 数组，默认需要确认，不会选中后立刻关闭。"><div :class="demoContentClass"><div><DemoDatePicker multiple :value="multiple" @change="setMultipleValue" /><p class="mt-space-sm w-full text-xs text-muted-foreground">当前值：{{ multiple.length ? multiple.map(getCalendarValueKey).join(', ') : '未选择' }}</p></div></div></Card>
        <Card title="RangePicker" description="范围选择使用专门入口，双面板复用 Calendar range state。"><div :class="demoContentClass"><div><DemoDatePicker range :value="range" @change="setRange" /><p class="mt-space-sm w-full text-xs text-muted-foreground">当前范围：{{ range.start ? getCalendarValueKey(range.start) : '空' }} ~ {{ range.end ? getCalendarValueKey(range.end) : '空' }}</p></div></div></Card>
        <Card title="动态范围禁用" description="选择一端后通过 disabledDate 限制另一端，只允许 7 天窗口。"><div :class="demoContentClass"><div><DemoDatePicker range :value="dynamicRange" :disabled-date="dynamicDisabled" @change="setDynamicRange" /><p class="mt-space-sm w-full text-xs text-muted-foreground">当前范围：{{ dynamicRange.start ? getCalendarValueKey(dynamicRange.start) : '空' }} ~ {{ dynamicRange.end ? getCalendarValueKey(dynamicRange.end) : '空' }}</p></div></div></Card>
