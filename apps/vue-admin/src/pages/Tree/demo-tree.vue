...
const props = withDefaults(defineProps<Props<TNode>>(), {
  checkable: false,
  virtual: false,
  height: 320,
  searchKeyword: '',
...
const options = computed<TreeOptions<TNode>>(() => ({
  ...
  ...(props.onSelectedKeysChange === undefined
    ? {}
    : { onSelectedKeysChange: props.onSelectedKeysChange }),
  ...(props.checkedKeys === undefined ? {} : { checkedKeys: props.checkedKeys }),
  ...(props.defaultCheckedKeys === undefined
    ? {}
    : { defaultCheckedKeys: props.defaultCheckedKeys }),
  ...(props.onCheckedKeysChange === undefined
    ? {}
    : { onCheckedKeysChange: props.onCheckedKeysChange }),
  ...(props.focusedKey === undefined ? {} : { focusedKey: props.focusedKey }),
  ...(props.defaultFocusedKey === undefined ? {} : { defaultFocusedKey: props.defaultFocusedKey }),
  ...(props.onFocusedKeyChange === undefined
    ? {}
    : { onFocusedKeyChange: props.onFocusedKeyChange }),
  ...(props.multiple === undefined ? {} : { multiple: props.multiple }),
  ...(props.onTreeDataChange === undefined ? {} : { onTreeDataChange: props.onTreeDataChange }),
}))
...
function scrollToKey(
  ...
  return virtualViewport.value?.scrollToKey(key, settings) ?? false
}
  ...
  <TreeRoot v-bind="rootBindings" :class="['w-full', props.class]" v-slot="{ tree }">
    ...
    >
      ...
      <DemoTreeRow
        ...
        v-bind="props.itemClass === undefined ? {} : { itemClass: props.itemClass }"
        :title-field="titleField"
        ><template #title="context"
          ><slot name="title" v-bind="context">{{
            String(context.item.node[titleField] ?? context.item.key)
          }}</slot></template
        ></DemoTreeRow
      >
    </TreeVirtualViewport>
