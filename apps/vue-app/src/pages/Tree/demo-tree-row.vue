...
<template>
  <TreeItem
    ...
    v-slot="{ item, itemProps, checkedState, loadState, actions }"
  >
    <div v-bind="itemProps">
      ...
      <Checkbox
        v-if="props.checkable && props.tree.hasFeature('check')"
        :checked="checkedState"
        :disabled="item.disabled"
        @click.stop
        @checked-change="actions.toggleChecked()"
      />
      <TreeTitle
        ><slot
          name="title"
          :item="item"
          :tree="props.tree"
          :is-searching="Boolean(props.searchKeyword)"
          :search-keyword="props.searchKeyword"
          >{{ String(item.node[props.titleField] ?? item.key) }}</slot
        ></TreeTitle
      >
    </div>
  </TreeItem>
