...
import { ConfirmFooter, DemoDatePicker, DemoRangePicker, DemoSection, RangePreview, ValuePreview } from './shared'
...
export function BasicDemos() {
  ...
  return (
    <>
      ...
      <DemoSection title="受控与非受控" description="value/onChange 管理受控值，defaultValue 提供非受控初始值。">
        <div>
          <DemoDatePicker value={controlled} onChange={(next) => setControlled(isValueArray(next) ? null : next)} />
          <ValuePreview value={controlled} />
        </div>
        <DemoDatePicker defaultValue={createCalendarDate(2026, 8, 1)} />
      </DemoSection>

      <DemoSection title="禁用日期" description="minDate/maxDate 限定固定区间，disabledDate 承载业务规则。">
      ...
      <DemoSection title="多选" description="multiple 使用 CalendarValue 数组，默认需要确认，不会选中后立刻关闭。">
        <div>
          <DemoDatePicker multiple value={multiple} onChange={setMultiple} footer={<ConfirmFooter />} />
          <ValuePreview value={multiple} />
        </div>
      </DemoSection>

      <DemoSection title="RangePicker" description="范围选择使用专门入口，双面板复用 Calendar range state。">
        <div>
      ...
      <DemoSection title="Prefix / Suffix" description="输入框复用 Input 能力，clear 和 suffix 互斥显示。">
        ...
        />
        <DemoDatePicker allowClear={false} triggerProps={{ suffix: <span className="text-xs">自定义</span> }} />
      </DemoSection>
    </>
  )
}
