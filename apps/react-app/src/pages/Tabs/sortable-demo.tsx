...
const sortableItemClassName =
  'cursor-grab touch-none active:cursor-grabbing data-[dragging=true]:opacity-60'
        ...
        {({ items: values }) => (
          <>
            ...
            <Sortable.SortableOverlay className="!box-border !inline-flex !h-7 !min-h-0 !items-center !rounded-md !border-0 !bg-muted-background !p-0 !text-muted-foreground !shadow-sm !outline-none !ring-0">
              {({ activeId }) => (
                <div className="flex h-7 items-center gap-1.5 px-2.5 text-sm">
                  <EllipsisIcon className="size-3 rotate-90" />
                  {byValue.get(String(activeId))?.label}
                </div>
              )}
