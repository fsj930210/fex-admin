...
{#snippet cell(item: Cell<Features, Person>)}{#if editing?.rowId === item.row.id && editing?.field === item.column.id}<Input autofocus value={String(item.getValue() ?? '')} type="text" onblur={event => update(item.row.id, item.column.id as Field, event.currentTarget.value)} onkeydown={event => { if (event.key === 'Enter') event.currentTarget.blur(); if (event.key === 'Escape') editing = null }} />{:else}<Button size="sm" variant="ghost" class="h-auto w-full justify-start px-0 text-inherit" onclick={() => editing = { rowId: item.row.id, field: item.column.id as Field }}>{String(item.getValue() ?? '')}</Button>{/if}{/snippet}
