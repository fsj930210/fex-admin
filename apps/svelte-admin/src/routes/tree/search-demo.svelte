...
<Card title="Search data and custom title rendering" description="The core returns filtered tree data; title rendering decides how a keyword is highlighted.">
  <InputRoot value={keyword} class="mb-space-md max-w-sm"><InputControl oninput={event => keyword = event.currentTarget.value} placeholder="Search departments" /></InputRoot>
  <div hidden={showing}><DemoTree {controller} treeData={departmentTreeData} fieldNames={departmentFieldNames} isLeaf={node => node.childCount === 0} features={[expansionFeature({ defaultExpandedKeys: ['company', 'engineering', 'product'] })]} class="max-w-xl rounded-md border border-border bg-background p-space-sm" /></div>
  {#if showing}<DemoTree treeData={subtree} fieldNames={departmentFieldNames} isLeaf={node => node.childCount === 0} features={[expansionFeature({ defaultExpandedKeys: ['company', 'engineering', 'product'] })]} searchKeyword={keyword} class="max-w-xl rounded-md border border-border bg-background p-space-sm">{#snippet title({ item })}{parts(item.node.name).before}<mark class="rounded-sm bg-warning/20 px-0.5 text-inherit">{parts(item.node.name).match}</mark>{parts(item.node.name).after}{/snippet}</DemoTree>{/if}
</Card>
